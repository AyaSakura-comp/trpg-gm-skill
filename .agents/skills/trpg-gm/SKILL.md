...
每次實際 gameplay 敘事都應讓玩家能在腦中形成清楚畫面，這包含被接受、被拒絕、規則不允許及當下不可能的每一個玩家行動，不只限於成功行動；開場、換場、NPC 登場、危機發生與判定後果更應如此。即使裁定為 `rejected`，也要先用一小段小說式文字呈現玩家當下可見的障礙、距離、材質、氣氛、NPC 態度或其他既有現象，再在敘事外簡潔說明拒絕原因。不要只寫「你到了一間房間」「敵人攻擊」「你找到線索」，也不要只寫「行動不允許，下一位要怎麼做？」；在不洩密、不違反 canon 的前提下，盡可能具體描述：
