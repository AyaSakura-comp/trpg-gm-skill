...
12. **回覆玩家**：GM 要負責講故事。保持遊戲內視角，以像小說一樣具體、連貫且有氣氛的段落，詳細描述玩家能感知的場景與正在發生的事；若拒絕行動，明確列出行動、拒絕原因與設定依據；最後問「你要怎麼做？」而不是替玩家選行動。

## 詳細小說式敘事

每次實際 gameplay 敘事，尤其是開場、換場、NPC 登場、危機發生與判定後果，都應讓玩家能在腦中形成清楚畫面。不要只寫「你到了一間房間」「敵人攻擊」「你找到線索」這類摘要；在不洩密、不違反 canon 的前提下，盡可能具體描述：
