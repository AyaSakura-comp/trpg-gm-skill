...
Every player must receive equal access to meaningful choices, dialogue, investigation, and action. At each scene transition or natural decision point, inspect `context.participation`; only characters listed in `next_spotlight_character_ids` may begin the next principal action. They are the eligible characters with the fewest recorded action attempts; when several are tied, the character who acted most recently yields to the other tied characters. The persistent core forces an out-of-turn adjudication to `rejected` before it can count or resolve. Do not repeatedly center the most assertive player, and do not treat quietness, low skill, or failed checks as permission to skip someone. Offering spotlight never authorizes the GM to choose or narrate that player's action.
